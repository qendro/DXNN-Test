...
prep(Agent_Id,PM_PId,OpMode)->
	...
	%% Initialize logging and log agent spawn
	agent_logger:init(),
	agent_logger:log_agent_spawn(Agent_Id),
	...
	log_agent_sensors(Agent_Id, SIds),
	...
	%% Log topology after spawning
	agent_logger:log_topology_spawned(Agent_Id, IdsNPIds, SIds, NIds, AIds),
	
	%% CRITICAL: Validate topology to prevent freeze bugs!
	try
		topology_validator:validate_spawned_topology(IdsNPIds, Agent_Id, SIds, AIds, NIds)
	catch
		exit:Reason ->
			agent_logger:log_validation_error(Agent_Id, Reason),
			io:format("~n~n===========================================~n"),
			io:format("TOPOLOGY VALIDATION FAILED for agent ~p~n", [Agent_Id]),
			io:format("Reason: ~p~n", [Reason]),
			io:format("===========================================~n~n"),
			%% STOP EVERYTHING - We need to debug why validation failed
			io:format("~n~n**** HALTING SYSTEM FOR DEBUGGING ****~n"),
			io:format("This agent's validation failure is blocking progress.~n"),
			io:format("Check logs to understand the root cause.~n~n"),
			exit({validation_failed, Agent_Id, Reason})
	end,
		...
		{Cx_PId,evaluation_completed,Fitness,Cycles,Time,GoalReachedFlag}->
			...
			case (U_Attempt >= S#state.max_attempts) or (GoalReachedFlag == true) of
				true ->	%End training
					...
					agent_logger:log_agent_terminate(S#state.agent_id, normal, U_HighestFitness, U_CycleAcc),
...
%%===============================Sensor Logging=======================================
%% Logs which sensors each agent is using for tracking and debugging purposes.

log_agent_sensors(Agent_Id, SIds) ->
	Sensor_Names = [get_sensor_name(SId) || SId <- SIds],
	Sensor_Summary = categorize_sensors(Sensor_Names),
	fx:log(io_lib:format("Agent ~p using sensors: ~s~n", [Agent_Id, Sensor_Summary])).

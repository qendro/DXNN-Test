...
construct_Cortex(Agent_Id,Generation,SpecCon,Encoding_Type,SPlasticity,SLinkform)->
	...
	case Encoding_Type of
		...
		substrate ->
			...
			[write(S) || S <- Sensors],
			[write(A) || A <- Actuators],
			Dimensions=calculate_OptimalSubstrateDimension(Sensors,Actuators), % Returns op
			Density = 5, % This is the density of the substrate. Which is the number of neurons in each layer.
			Depth = 1, % This is the depth of the substrate. which is the number of layers in the substrate.
...
calculate_OptimalSubstrateDimension(Sensors,Actuators)->
		S_Formats = [S#sensor.format || S<-Sensors], %S_Formats is a list of the formats of all the sensors for the substrate.
		fx:log("S_Formats:~p~n",[S_Formats]),

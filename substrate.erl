	...
	receive
		{ExoSelf,init,InitState}->
			try
				...
				Error:Reason:Stack ->
					io:format("~n~n********SUBSTRATE CRASH ON INIT********~n"),
					io:format("Substrate PID: ~p~n", [self()]),
					io:format("Error: ~p~n", [Error]),
					io:format("Reason: ~p~n", [Reason]),
					io:format("Stack: ~p~n", [Stack]),
					io:format("InitState: ~p~n", [InitState]),
					io:format("****************************************~n~n"),
					exit({substrate_init_crash, Error, Reason})
			end
	after 5000 ->
		io:format("~n~n********SUBSTRATE TIMEOUT - NO INIT MESSAGE RECEIVED********~n"),
		io:format("Substrate PID: ~p~n", [self()]),
		io:format("ExoSelf: ~p~n", [ExoSelf]),
		io:format("********************************************************~n~n"),
		exit(substrate_init_timeout)
	end.

loop(ExoSelf,S,[SPId|SPIds],SAcc)->
	receive
		{SPId,forward,Sensory_Signal}->
			agent_logger:log_message(ExoSelf, substrate, self(), received, {SPId, forward}),
		...
		{_AnySPId,forward,Sensory_Signal} ->
			...
			CurrentList = [SPId|SPIds],
			agent_logger:log_message(ExoSelf, substrate, self(), received_outoforder, {_AnySPId, expected, CurrentList}),
			case lists:member(_AnySPId, CurrentList) of
				true ->
					U_SPIds = lists:delete(_AnySPId, CurrentList),
					loop(ExoSelf,S,U_SPIds,[Sensory_Signal|SAcc]);
				false ->
					agent_logger:log_message(ExoSelf, substrate, self(), deadlock_detected, {_AnySPId, CurrentList}),
					io:format("~n~n********SUBSTRATE DEADLOCK DETECTED********~n"),
					io:format("Substrate ~p received message from UNKNOWN sensor ~p~n", [self(), _AnySPId]),
					io:format("Expected sensors: ~p~n", [CurrentList]),
					io:format("Already received: ~p signals~n", [length(SAcc)]),
					io:format("This would cause infinite wait - EXITING~n"),
					io:format("*******************************************~n~n"),
					exit({substrate_unknown_sensor, _AnySPId, CurrentList})
			end;
...
%			io:format("Resulting substrate:~p~n",[Substrate]),
			void;
		Msg ->
			io:format("********ERROR: SUBSTRATE got unexpected message: ~p~n", [Msg]),
			loop(ExoSelf,S,[SPId|SPIds],SAcc)
%		after 20000 ->
%			io:format("********ERROR: Substrate Crashed:~p~n",[S])
	end;
loop(ExoSelf,S,[],SAcc)->%All sensory signals received
	agent_logger:log_message(ExoSelf, substrate, self(), all_sensors_received, length(SAcc)),
	{U_Substrate,U_SMode,OAcc} = reason(SAcc,S),
	agent_logger:log_message(ExoSelf, substrate, self(), sending_to_actuators, S#state.apids),
